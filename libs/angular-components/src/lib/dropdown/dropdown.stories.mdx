...
<Canvas>
  ...
  >
    {(args) => {
      ...
      return {
        template: `<span style="width:300px;display:inline-block;">
                    <goa-dropdown1 (selectionChange)="selectionChanged($event)"
                        [required]="required"
                        [menuHeight]="menuHeight"
                        [typeaheadMode]="typeaheadMode"
                        [disabled]="disabled"
...
```html
<goa-dropdown1 (selectionChange)="selectionChanged($event)" label="Fruits" description="Select your favourite fruit.">
  <goa-option id='apples' [value]='{fruit: "apples"}' label='Apples'>Apples</goa-option>
